...
/* c8 ignore start */
server.register(config.NODE_ENV === "test" ? require("fastify-redis-mock") : require("@fastify/redis"), {
  ...
  if (error.validation) {
    ...
    return reply.status(400).send({
      ...
      errors: error.validation.map((err) => ({
        key: err.params && err.params.missingProperty ? err.params.missingProperty : err.instancePath.replace(new RegExp("/", "g"), ""),
        value: err.message,
      })),
    });
  }
